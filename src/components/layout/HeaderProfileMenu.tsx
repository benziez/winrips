  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className="flex h-9 max-w-[9.5rem] shrink-0 items-center justify-center gap-1 rounded-md border border-border bg-slate text-white transition-colors duration-200 hover:border-fuchsia/40 hover:bg-slate-elevated sm:max-w-[11rem] md:h-auto md:w-auto md:gap-1.5 md:px-4 md:py-2 md:text-xs md:font-bold md:uppercase md:tracking-wider"
      ...
      >
        ...
        <span className="min-w-0 truncate text-[11px] font-semibold normal-case tracking-normal sm:text-xs md:max-w-[8rem]">
          {profileLoading && !profileUsername && !metadataUsername ? "Profile" : displayLabel}
        </span>
        <ChevronDown
          size={14}
      ...
      {menuOpen ? (
        <div
          role="menu"
          className="absolute right-0 z-[60] mt-2 w-52 rounded-md border border-[#213743] bg-[#1a2c38] py-2 shadow-xl"
        >
          <div
