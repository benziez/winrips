...
import { APPLE_SIGN_IN_CANCELLED } from "../../lib/appleAuth";
...
export function AppleSignInButton({ onSuccess, onError, className = "" }: AppleSignInButtonProps) {
  ...
  async function handleClick() {
    ...
    try {
      ...
      if (cancelled || error === APPLE_SIGN_IN_CANCELLED) {
        return;
      }
      if (error) {
        onError?.(error);
  ...
  return (
    <button
      ...
      disabled={submitting}
      className={`flex h-12 w-full items-center justify-center gap-2.5 overflow-visible rounded-xl border border-white/15 bg-white px-4 text-sm font-semibold leading-none text-black transition-opacity disabled:opacity-50 ${className}`}
    >
      <svg
        width="18"
        height="18"
