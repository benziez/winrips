-- Infinite Series: ensure boxes rows exist + open_pack returns JSON errors (no silent raise).
-- After applying, run: npx vite-node scripts/syncPackWeights.ts
...
-- Patch open_pack: missing weights / winner must return JSON (not raise) so the client can toast + stay on lobby.
