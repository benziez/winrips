-- Exempt wotc-first-edition from global daily open cap checks in open_pack.
-- Re-applies the full function from supabase/open_pack.sql with the whitelist.
...
comment on function public.open_pack(uuid, text, integer, integer, boolean) is
  'Atomic pack open: server-side pack cost from boxes, weighted draw from pack_card_weights, gem deduction, and vault insert. wotc-first-edition bypasses global daily open cap.';
